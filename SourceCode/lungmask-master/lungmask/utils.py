...
def preprocess(img, label=None, resolution=[192, 192]):
    ...
    imgmtx[imgmtx > 600] = 600 # 筛选数据
    cip_xnew = []
    cip_box = []
    cip_mask = []
    for i in range(imgmtx.shape[0]):
        if label is None:
            (im, m, box) = crop_and_resize(imgmtx[i, :, :], width=resolution[0], height=resolution[1]) # img, mask, bbox
        else:
...
def crop_and_resize(img, mask=None, width=192, height=192):
    ...
    # skimage.measure.regionprops()函数 筛选部分连通区域
    if len(reg) > 0:
...
def get_input_image(path):
    if os.path.isfile(path):
        ...
        dicom_vols = read_dicoms(path, original=False, primary=False) # 此处读取数据
        if len(dicom_vols) < 1:
