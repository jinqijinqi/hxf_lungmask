...
def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    if model is None:
        model = get_model('unet', 'R231')# 默认模型

    inimg_raw = sitk.GetArrayFromImage(image)# 读取输入图像
    directions = np.asarray(image.GetDirection()) # 读取文件夹
    if len(directions) == 9:
...
# 设备信息
    ...
    if not noHU:
        tvolslices, xnew_box = utils.preprocess(inimg_raw, resolution=[256, 256]) # img, bbox
